...
`;
		...
		*/
		function hasPendingInteraction(uiSession, sessions, skipSubagents = true) {
		...
		*/
		function resolvePetState(host, agent, pending, demo) {
		...
		* （boot 在 StrictMode/HMR 下会重复执行，避免二次注入与重复初始化）。 */
		...
		function PetAnchor(props) {
			const ref = (0, react.useRef)(null);
			const { ctx } = props;
			(0, react.useEffect)(() => boot(ref.current, ctx), [ctx]);
			return (0, react.createElement)("div", {
				ref,
				style: {
					width: 0,
		...
		*/
		...
		function boot(anchor, ctx) {
			...
			const pendingFiber = ctx.inject(["uiSession"], (uiCtx) => {
				const uiSession = uiCtx.get("uiSession");
				const store = uiSession?.pendingInteractions;
				if (store === void 0) return;
				const sessions = uiCtx.get("sessions");
				pending.available = true;
				pending.active = hasPendingInteraction(uiSession, sessions, true);
				if (bubble) applyState(view);
				const unsubscribePending = store.subscribe(() => {
					const next = hasPendingInteraction(uiSession, sessions, true);
			...
			* 避免在 MotionManager.update 中间重入修改 MotionState。 */
			...
			function applyState(next) {
				const state = resolvePetState(next?.state ?? "idle", next?.agent ?? "idle", pending, demoState);
		...
		function apply(ctx) {
			...
			slots.inject("shell.overlay", () => slots.register({
				name: "shell.overlay",
				id: "anime25d-pet"
			}, () => (0, react.createElement)(PetAnchor, { ctx })));
